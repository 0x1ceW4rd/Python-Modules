...
class DataProssesor:
    def __init__(self, data):
        self.data = data
        self.lenof_data = len(data)

    def data_stream_prossesor(self, to_print):
        print("=== Game Data Stream Processor ===\n")
        print(f"Processing {self.lenof_data} game events...\n")

        for data in self.data:
            eventid = data["id"]
            eventplayer = data["player"]
            eventplayerlvl = data["data"]["level"]
            eventtype = data["event_type"]
            if to_print != 0:
                yield (f"Event {eventid}: Player {eventplayer} "
                       f"(level {eventplayerlvl}) {eventtype}")
                to_print -= 1
        if to_print != self.lenof_data:
            print("...", end="\n\n")

    def data_analyser(self):
        print("=== Stream Analytics ===\n")

        p = [lvl for lvl in self.data if lvl['data']['level'] > 10]
        e = [lvlup for lvlup in self.data if lvlup['event_type'] == 'level_up']
        print(f"Total events processed: {self.lenof_data}")
        print(f"High-level players (10+): "
              f"{len(p)}")
...
if __name__ == "__main__":
    ...
    prossesor = DataProssesor(data_to_prosses)
    s = prossesor.data_stream_prossesor(3)
    for g in iter(s):
        print(g)
    prossesor.data_analyser()
